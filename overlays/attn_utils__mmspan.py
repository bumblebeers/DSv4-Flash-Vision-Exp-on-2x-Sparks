...
def compute_mm_prefix_ranges(
    ...
    from attending across the entire image span, unless the model clamps
    mm prefixes to the sliding window in-kernel (e.g. DeepSeek-V4 vision
    and Gemma4), in which case the range is kept and the kernel bounds it
    per query.

    Models with ``mm_prefix_span_leading_pad_modulus`` (e.g. DeepSeek-V4
    vision) define the bidirectional span over the whole sentinel block
    [IMAGE_START, IMAGE_END] with a position-dependent alignment pad
    prepended before the first sentinel. For those, derive the span from
    the full placeholder range with the pad stripped, instead of the
    embed-token ranges (which, in the N-layout, are per-row-pair runs).
    """
